...
class DIEN_NOMASK(tf.keras.Model):
    def __init__(self, user_count, item_count, cate_count, cate_list,
                       user_dim, item_dim, cate_dim,
                       dim_layers):
        self.item_dim = item_dim
    ...
    def call(self, user, item, cate_history, item_history, length):
        user_emb, item_join_emb, item_bias, hist_join_emb = self.get_emb(user, item, cate_history, item_history)
    ...
    def get_emb(self, user, item, cate_history, item_history):
        user_emb = self.user_emb(user)
        ...
        hist_emb = self.item_emb(item_history)
        hist_cate_emb = self.cate_emb(tf.gather(self.cate_list, cate_history))
        hist_join_emb = tf.concat([hist_emb, hist_cate_emb], -1)
